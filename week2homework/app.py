...
def solve_sle(x_values, y_values):
    ...
    return np.poly1d(coefficients[::-1])  # revert
...
def plot_polynomials(polynomials, x_values, y_values, interval, labels):
    ...
    for i, poly in enumerate(polynomials):
        if isinstance(poly, tuple):  # Parametric case
            t_plot = np.linspace(0, len(x_values) - 1, 400)
...
def calculate_polynomial_value(polynomials, x, labels, x_values):
    ...
    for i, poly in enumerate(polynomials):
        if isinstance(poly, tuple):  # Parametric case
            ...
            print(f"  {labels[i]}:  x(t) ≈ {poly[0](closest_t):.4f}, y(t) ≈ {poly[1](closest_t):.4f}  (using approximate t ≈ {closest_t:.4f})")
        else:
    ...
    if input_method == '1':  # Function Input
        while True:
            try:
                ...
                print("Example of a valid expression: x * sin(x) - x**2 + 1")
        while True:
            try:
        ...
        interval = (min(x_values), max(x_values))  # Plotting range based on file points.
    ...
    while True:
        try:
            ...
            if x_input.lower() == 'q':
                break
            x_val = float(x_input)
            if not interval[0] <= x_val <= interval[1]:
                print(f"Warning: x = {x_val} is outside the interpolation interval {interval}.")
            calculate_polynomial_value(polynomials, x_val, labels, x_values)
        except ValueError:
